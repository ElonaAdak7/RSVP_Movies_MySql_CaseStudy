...
SELECT 
        ...
        SUM(CASE WHEN  WORLWIDE_GROSS_INCOME IS NULL THEN 1 ELSE 0 END ) AS WORLWIDE_NULLS ,
...
SELECT COUNT(ID) NUMBER_OF_MOVIES , COUNTRY , YEAR
FROM MOVIE
...
SELECT  GENRE , COUNT(MOVIE_ID) AS HIGHEST_NUMBER_OF_MOVIES , YEAR
FROM GENRE 
INNER JOIN MOVIE 
ON GENRE.MOVIE_ID = MOVIE.ID
WHERE YEAR = 2019 
GROUP BY GENRE ,YEAR
ORDER BY HIGHEST_NUMBER_OF_MOVIES DESC LIMIT 3;
...
SELECT * FROM ratings ;   # Just to see the all columns of ratings for analysis 
...
JOIN  ratings
ON   
   movie.id = ratings.movie_id AND production_company IS NOT NULL
WHERE avG_rating > 8
...
WHERE
      total_votes > 1000 AND year = 2017 AND MONTH(date_published) = 3 AND country = 'USA'
GROUP BY 
          genre
...
ORDER BY avg_rating DESC;
 ...
 /* You should also try your hand at median rating and check whether the ‘median rating’ column gives 
any significant insights.
-- Q16. Number of the movies released between 1 April 2018 and 1 April 2019, how many were given a
 median rating of 8? */
-- Type your code below:
...
ON
   movie.id = ratings.movie_id
WHERE 
     date_published BETWEEN '2018-04-1' AND '2019-04-01' AND median_rating = 8 
GROUP BY 
       median_rating ;
...
GROUP BY country
 ORDER BY total_num_of_votes DESC ;

-- Answer is Yes

/* Now that you have analysed the movies, genres and ratings tables, let us now analyse another table, 
the names table. 
...
/* James Mangold can be hired as the director for RSVP's next project. Do you remeber his movies, 
...
WITH top_actor AS
(  SELECT
         name AS actor_name , SUM(total_votes) AS total_votes , COUNT(DISTINCT role_mapping.movie_id ) AS movie_count , 
...
-- (Hint: You should use the weighted average based on votes. If the ratings clash, then the total number of votes should act as the 
tie breaker.) */
/* Output format:
+---------------+-------------------+---------------------+----------------------+-----------------+
...
WHERE  category = 'actress' AND languages LIKE '%Hindi%' AND country = 'India'
...
SELECT
      title ,
      CASE  WHEN avg_rating > 8 THEN 'SuperHit_Movies' 
...
/* -- Finally, let’s find out the names of the top two production houses that have produced the highest number of hits among 
multilingual movies.
